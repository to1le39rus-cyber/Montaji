...
function toast(text, kind='normal') {
  ...
  clearTimeout(toast.timer); toast.timer = setTimeout(() => el.remove(), 3400);
}
function dataFromSnap(snap) { const data = snap?.exists() ? snap.data()?.data || {} : {}; return Array.isArray(data.notes) ? data.notes : []; }
...
function render() {
  ...
  if (active && archive) {
    const open = notes.filter(n => n.archived !== true), old = notes.filter(n => n.archived === true);
...
function renderUrgentInsights() {
  ...
  urgentNotes().slice(0, 4).reverse().forEach(n => {
    ...
    el.className = 'insight note-insight note-insight-urgent'; el.dataset.noteOpenUrgent = n.id;
...
function openNote(note = null, viewOnly = false) {
  const m = document.createElement('div'); m.className = 'modal open notes-v2-modal';
  if (viewOnly && note) {
    m.innerHTML = `<div class="backdrop"></div><div class="sheet"><div class="handle"></div><div class="sheet-head"><div><div class="eyebrow">${note.urgent ? '⚑ СРОЧНАЯ ЗАМЕТКА' : 'ЗАМЕТКА'}</div><h2>${esc(note.title || 'Заметка')}</h2></div><button class="circle-btn" data-note-close>×</button></div>
      <div class="note-detail-meta"><span>${esc(note.authorName || note.authorEmail || 'Пользователь')}</span>${note.dueDate ? `<span>до ${esc(fmtDate(note.dueDate))}</span>` : ''}${note.done ? '<span>✓ Сделано</span>' : ''}</div>
      <div class="note-detail-text">${esc(note.text || 'Без текста')}</div>
      <div class="note-detail-actions"><button type="button" class="mini-btn" data-note-edit-detail="${esc(note.id)}">Редактировать</button><button type="button" class="mini-btn" data-note-archive-detail="${esc(note.id)}">В архив</button></div>
    </div>`;
  } else {
    m.innerHTML = `<div class="backdrop"></div><div class="sheet"><div class="handle"></div><div class="sheet-head"><div><div class="eyebrow">Общие заметки</div><h2>${note ? 'Изменить заметку' : 'Новая заметка'}</h2></div><button class="circle-btn" data-note-close>×</button></div>
      ...
      <div class="form-row"><label>Сделать до<input id="notesV2Due" type="date" value="${esc(note?.dueDate || '')}"></label><label class="note-form-check"><input id="notesV2Done" type="checkbox" ${note?.done ? 'checked' : ''}><span>Сделано</span></label></div>
      <label class="note-form-check note-form-urgent"><input id="notesV2Urgent" type="checkbox" ${note?.urgent ? 'checked' : ''}><span>⚑ Срочная заметка</span></label>
      <button class="primary wide" type="submit">${note ? 'Сохранить' : 'Добавить заметку'}</button></form></div>`;
  }
  document.body.append(m);
  m.querySelector('.backdrop').onclick = () => closeModal(m); m.querySelector('[data-note-close]').onclick = () => closeModal(m);
  ...
  const form = $('#notesV2Form', m); if (!form) return;
  form.onsubmit = async e => {
    e.preventDefault(); const title = $('#notesV2Title', m).value.trim(), text = $('#notesV2Text', m).value.trim(); if (!title) return;
    const item = { id: note?.id || uid(), title, text, dueDate: $('#notesV2Due', m).value || '', done: $('#notesV2Done', m).checked, urgent: $('#notesV2Urgent', m).checked, archived: note?.archived === true, authorUid: note?.authorUid || currentUser.uid, authorName: note?.authorName || currentUser.displayName || currentUser.email || 'Пользователь', authorEmail: note?.authorEmail || currentUser.email || '', createdAt: note?.createdAt || new Date().toISOString(), updatedAt: new Date().toISOString() };
...
async function writeNotes(mutator) {
  if (!currentUser) throw new Error('AUTH_REQUIRED'); const ref = F.doc(db, ...NOTES_DOC);
  await F.runTransaction(db, async tx => { const snap = await tx.get(ref); const next = await mutator({ notes: dataFromSnap(snap) }); tx.set(ref, { data: { notes: Array.isArray(next.notes) ? next.notes : [] }, version: 2, updatedAt: F.serverTimestamp(), updatedBy: currentUser.uid }, { merge: true }); });
}
...
function bindEvents() {
  document.addEventListener('click', e => {
    const add = e.target.closest('#todayNoteBtn,[data-add-type="Заметка"],[data-note-open-v2]'); if (add) { e.preventDefault(); e.stopPropagation(); openNote(); return; }
    const urgentOpen = e.target.closest('[data-note-open-urgent]'); if (urgentOpen) { e.preventDefault(); openNote(notes.find(n => n.id === urgentOpen.dataset.noteOpenUrgent), true); return; }
    const open = e.target.closest('[data-note-open]'); if (open) { e.preventDefault(); openNote(notes.find(n => n.id === open.dataset.noteOpen), true); return; }
    const edit = e.target.closest('[data-note-edit-v2]'); if (edit) { e.preventDefault(); e.stopPropagation(); const n = notes.find(x => x.id === edit.dataset.noteEditV2); if (n) openNote(n); return; }
    const archive = e.target.closest('[data-note-archive-v2]'); if (archive) { e.preventDefault(); e.stopPropagation(); patchNote(archive.dataset.noteArchiveV2, { archived: true }); return; }
    const restore = e.target.closest('[data-note-restore-v2]'); if (restore) { e.preventDefault(); e.stopPropagation(); patchNote(restore.dataset.noteRestoreV2, { archived: false }); return; }
  }, true);
...
async function start() {
  try {
    ...
    const app = appMod.getApps().find(x => x.name === 'montaji-aa-production'); if (!app) return;
...
.notes-empty{display:grid;gap:5px;padding:12px 0 6px}.notes-empty strong{font-size:14px}.notes-empty span{font-size:12px;opacity:.58}.notes-v2-modal .sheet{max-height:90vh;overflow:auto}
.notes-v2-modal .form-row{align-items:end}.notes-v2-modal .note-form-check{display:flex;align-items:center;gap:9px;min-height:48px;white-space:nowrap}.notes-v2-modal .note-form-check input{appearance:none!important;-webkit-appearance:none!important;width:22px!important;height:22px!important;min-width:22px!important;margin:0!important;border:1.5px solid #a9aaa7!important;border-radius:7px!important;background:#fff!important;opacity:1!important;position:static!important;box-sizing:border-box!important}.notes-v2-modal .note-form-check input:checked{border-color:#5b63d9!important;background:#5b63d9!important}.notes-v2-modal .note-form-check input:checked::after{content:'✓';display:block;text-align:center;line-height:19px;font-size:13px;color:#fff;font-weight:700}.notes-v2-modal .note-form-check span{font-weight:600;color:#72766f}.notes-v2-modal .note-form-urgent{margin-top:10px;padding:11px 12px;border-radius:12px;background:#fff7f7}.notes-v2-modal .note-form-urgent span{color:#c94242}
.note-insight{width:100%;display:flex;align-items:center;gap:10px;text-align:left;cursor:pointer}.note-insight-flag{color:#c94242;font-size:18px;line-height:1}.note-insight-copy{display:grid;gap:2px;flex:1;min-width:0}.note-insight-copy strong{font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.note-insight-copy small{font-size:11px;opacity:.6}.note-insight-arrow{font-size:23px;opacity:.5}
.note-detail-meta{display:flex;flex-wrap:wrap;gap:7px;margin:0 0 16px;color:#777b74;font-size:12px}.note-detail-meta span{padding:6px 9px;background:#f3f4f0;border-radius:9px}.note-detail-text{padding:16px;border:1px solid #e2e3df;border-radius:14px;white-space:pre-wrap;line-height:1.5;font-size:15px;background:#fff}.note-detail-actions{display:flex;justify-content:flex-end;gap:7px;margin-top:14px}
`;
