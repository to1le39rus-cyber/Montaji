(() => {
  const STYLE = `
    #moneyScreen .debt-minimal{box-sizing:border-box!important;width:100%!important;margin:14px 0 0!important;padding:0!important;min-height:0!important;height:auto!important;border:0!important;border-radius:0!important;box-shadow:none!important;background:transparent!important;overflow:visible!important;}
  ...
  function debtCard(){const list=document.querySelector('#debtList');return list?.closest('.chart-card')||null;}
  function expenseCard(){const list=document.querySelector('#expenseArchive');return list?.closest('.chart-card')||null;}
  ...
  function apply(){
    ...
    const expense=expenseCard();
    if(expense&&expense.parentElement===screen&&expense.nextElementSibling!==card){expense.insertAdjacentElement('afterend',card);}
    else if(!expense&&card.parentElement===screen&&card!==screen.lastElementChild){screen.appendChild(card);}
    const list=document.querySelector('#debtList');
    if(!list)return;
    const hasDebt=list.querySelector('.debt-row'),muted=list.querySelector('.muted');
    if(!hasDebt&&muted&&muted.textContent.includes('Долгов нет'))muted.textContent='Нет долгов';
  }
  let timer=0;
