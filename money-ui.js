(() => {
  ...
  const periodStart = type => { const now = new Date(); if (type === 'month') return new Date(now.getFullYear(), now.getMonth(), 1); if (type === 'week') { const monday=(now.getDay()+6)%7,d=new Date(now); d.setHours(12,0,0,0); d.setDate(now.getDate()-monday); return d; } return null; };
  ...
  const matches = row => { if(state.category!=='Все'&&rowCategory(row)!==state.category)return false; if(state.period!=='all'){const d=rowDate(row),start=periodStart(state.period);if(!d||!start||d<start||d>new Date())return false;} return true; };

  function ensureLunchOption(){ document.querySelectorAll('select').forEach(select=>{const opts=[...select.options].map(o=>o.textContent.trim());const looksLikeExpense=opts.some(x=>/Аренда|Материалы|Топливо|Другое|Прочее|Расход/i.test(x));if(looksLikeExpense&&!opts.includes('Обед')){const option=document.createElement('option');option.value='Обед';option.textContent='Обед';const other=[...select.options].find(o=>/Другое|Прочее/i.test(o.textContent));if(other)select.insertBefore(option,other);else select.append(option);}});}
  ...
  function makeUI(box){
    ...
    wrap.innerHTML=`<div class="money-ui-head"><div><strong>Расходы</strong><small>последние 15 · фильтр по категории и периоду</small></div><button type="button" class="money-ui-reset" hidden>Сбросить</button></div><div class="money-ui-periods" role="tablist" aria-label="Период">${PERIODS.map(([v,l])=>`<button type="button" data-money-period="${v}" class="${v==='all'?'selected':''}">${l}</button>`).join('')}</div><div class="money-ui-cats" aria-label="Категория"></div><div class="money-ui-summary"></div><button type="button" class="money-ui-more" hidden></button>`;
    box.parentNode.insertBefore(wrap,box);
    wrap.addEventListener('click',e=>{const p=e.target.closest('[data-money-period]');if(p){state.period=p.dataset.moneyPeriod;state.showAll=false;apply();return;}const c=e.target.closest('[data-money-category]');if(c){state.category=c.dataset.moneyCategory;state.showAll=false;apply();return;}if(e.target.closest('.money-ui-more')){state.showAll=!state.showAll;apply();return;}if(e.target.closest('.money-ui-reset')){state.category='Все';state.period='all';state.showAll=false;apply();}});
  }

  function renderCategories(wrap,rs){const cats=[...new Set(rs.map(rowCategory))].filter(Boolean).sort((a,b)=>a.localeCompare(b,'ru'));wrap.querySelector('.money-ui-cats').innerHTML=['Все',...cats].map(c=>`<button type="button" data-money-category="${esc(c)}" class="${state.category===c?'selected':''}">${esc(c)}</button>`).join('');}

  function apply(){
    const box=document.querySelector('#expenseArchive'),wrap=document.querySelector('.money-ui');if(!box||!wrap)return;
    observer?.disconnect();
    try{
      const rs=rows();renderCategories(wrap,rs);const filtered=rs.filter(matches),visible=state.showAll?filtered:filtered.slice(0,MAX_VISIBLE);
      ...
      const label=state.category==='Все'?'Все категории':state.category,period=PERIODS.find(x=>x[0]===state.period)?.[1]||'Все';
  ...
  observer=new MutationObserver(mutations=>{if(mutations.every(m=>m.target.closest?.('.money-ui')))return;ensureLunchOption();const archive=document.querySelector('#expenseArchive');if(archive){makeUI(archive);apply();}});
