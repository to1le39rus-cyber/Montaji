...
export const createCanonicalShell = ({
  ...
  function openPartnerSchedule(request,returnTo) {
    ...
    form.onsubmit=async e=>{e.preventDefault();try{const values=Object.fromEntries(new FormData(form));await partnerRequestService.assign(request.id,values);if(canJobs){const existing=state.snapshot.state.jobs.find(job=>job.storeRequestId===request.id);if(!existing){const created=await jobService.create({date:values.date,time:values.time||'',type:request.kind==='installation'?'Монтаж':'Замер',slot:'4',status:'Запланировано',paid:false,client:request.client,phone:request.phone,address:request.address,source:request.organizationName||request.storeName||'Магазин',comment:request.managerComment||'',storeRequestId:request.id,storeId:request.organizationId||'',storeNameSnapshot:request.organizationName||request.storeName||''});const current=state.snapshot.state;state.setState({...current,jobs:[...current.jobs,created]})}}notify(request.kind==='installation'?'Монтаж назначен':'Замер назначен');returnTo()}catch(error){notify(error.message)}};
