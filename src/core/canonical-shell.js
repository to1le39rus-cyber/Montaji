...
export const createCanonicalShell=({root,initialState={jobs:[],expenses:[],version:5},initialNotes=[],user=null,readOnly=false,jobService=null})=>{
 ...
 async function mutateJob(id,operation){ if(readOnly||!jobService) return false; try{ await operation(id); return true; }catch(error){ console.error(error); alert(error?.message||'Не удалось сохранить заявку'); return false; } }
 ...
 for(const [label,fn] of commands){const b=document.createElement('button');b.type='button';b.textContent=label;b.addEventListener('click',async()=>{const result=await fn();if(result!==null&&result!==false)close()});actions.append(b)}
}
