...
export const createCanonicalShell=({root,initialState={jobs:[],expenses:[],version:5},initialNotes=[],user=null,readOnly=false,jobService=null})=>{
 ...
 async function mutateJob(id,operation){ if(readOnly||!jobService) return; try{ await operation(id); }catch(error){ console.error(error); alert(error?.message||'Не удалось сохранить заявку'); } }
 function openReschedule(job){
  ...
  panel.querySelector('[data-save]').onclick=async()=>{const date=panel.querySelector('input').value;if(!date)return;await mutateJob(job.id,(id)=>jobService.reschedule(id,date));close()}; modal.append(panel);
 }
 function openJob(job={}){
  modal.hidden=false;modal.innerHTML='';const panel=document.createElement('section');panel.className='canonical-modal-panel';
  const close=()=>{modal.hidden=true;modal.innerHTML=''};
  if(readOnly)return;
  const form=createJobForm({job,onCancel:close,onSubmit:async next=>{if(job.id) await mutateJob(job.id,(id)=>jobService.update(id,next)); else if(jobService) await jobService.create(next); close();}});
 ...
 for(const [label,fn] of commands){const b=document.createElement('button');b.type='button';b.textContent=label;b.addEventListener('click',async()=>{await fn();close()});actions.append(b)}
}
