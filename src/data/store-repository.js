...
export const createStoreRepository=({firestore,doc,runTransaction,serverTimestamp,collectionPath=['appData','shared'],actorId='client'})=>{
 ...
 const update=(id,transform)=>runTransaction(firestore,async tx=>{const {current,stores}=read(await tx.get(ref));const i=stores.findIndex(x=>x?.id===id);if(i<0)throw new Error('Магазин не найден');const fresh=normalizeStore(stores[i]);const patch=typeof transform==='function'?await transform(fresh):transform;const value=normalizeStore({...fresh,...patch,id});if(!value.name)throw new Error('Укажите название магазина');write(tx,current,stores.map((x,n)=>n===i?value:x));return value});
