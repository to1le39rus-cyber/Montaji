...
export const renderSchedule=({root,model,onJobClick=()=>{},onDateChange=()=>{}})=>{
 if(!root)return;
 root.innerHTML='';
 ...
 const dateInput=document.createElement('input'); dateInput.type='date'; dateInput.value=model.date; dateInput.className='screen-date-input';
 dateInput.setAttribute('aria-label','Дата расписания'); dateInput.addEventListener('change',()=>onDateChange(dateInput.value));
 header.append(dateInput); root.append(header);
 const summary=document.createElement('div'); summary.className='schedule-summary';
 summary.innerHTML='<span>Монтажей: <strong>'+model.montageCount+'</strong></span><span>Выполнено: <strong>'+model.completedCount+'</strong></span>'; root.append(summary);
 const list=document.createElement('section'); list.className='schedule-list';
 for(const job of model.jobs){
  const row=document.createElement('button'); row.type='button'; row.className='schedule-row'; row.dataset.jobId=job.id;
  ...
  row.addEventListener('click',()=>onJobClick(job)); list.append(row);
 }
