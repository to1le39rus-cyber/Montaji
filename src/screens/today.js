...
export const buildTodayModel = ({state,date,notes=[]}) => {
  ...
  const yesterday=new Date(date+'T12:00:00');yesterday.setDate(yesterday.getDate()-1);const yesterdayKey=yesterday.toISOString().slice(0,10);
  const overdue=sortBySchedule(allJobs.filter(j=>j.type==='Монтаж'&&j.date===yesterdayKey&&!isCompleted(j)&&!isCancelled(j)));
