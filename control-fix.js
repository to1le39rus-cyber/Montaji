/* MONТАЖИ — operational control queue */
(async()=>{
  const V='10.14.1';
  try{
    ...
    const write=async controls=>{const u=auth.currentUser;if(!u)throw Error('NO_AUTH');await fs.setDoc(ref,{data:{controls},version:1,updatedAt:fs.serverTimestamp(),updatedBy:u.uid},{merge:true})};
    ...
    const render=async()=>{
      ...
      if(!sec){
        const anchor=document.querySelector('#insights')?.parentElement;
        if(!anchor)return;
      ...
      list.innerHTML=controls.map(n=>`<article class="control-card ${due(n)?'is-due':''}"><div class="control-main"><div class="control-title">${esc(n.title||n.client||'Контроль')}</div><div class="control-meta">${n.checkDate?(due(n)?'⚠️ Проверить сегодня':'Проверить '+esc(fmt(n.checkDate))):'Дата проверки не задана'}</div>${n.text?`<div class="control-text">${esc(n.text)}</div>`:''}</div><div class="control-actions"><button data-control-call="${esc(n.phone||'')}">Позвонить</button><button data-control-check="${esc(n.id)}">Проверено</button>${n.jobId?`<button data-control-job="${esc(n.id)}">Назначить дату</button>`:''}</div></article>`).join('');
      bind();
      const add=sec.querySelector('#addControlBtn');if(add&&!add.dataset.bound){add.dataset.bound='1';add.onclick=()=>openControl();}
    };
    const bind=()=>{
      document.querySelectorAll('[data-control-call]').forEach(b=>{if(b.dataset.bound)return;b.dataset.bound='1';b.onclick=()=>{const p=(b.dataset.controlCall||'').replace(/[^\d+]/g,'');if(p)location.href=`tel:${p}`;else toast('Телефон не указан','error')}});
      document.querySelectorAll('[data-control-check]').forEach(b=>{if(b.dataset.bound)return;b.dataset.bound='1';b.onclick=async()=>{const id=b.dataset.controlCheck,cs=await read(),n=cs.find(x=>x.id===id);if(!n)return;const next=prompt('Что выяснили?',n.lastResult||'');if(next===null)return;const ready=confirm('Дверь уже готова и можно назначать дату?');if(ready&&n.jobId){await openJobForControl(n);return}const d=prompt('Когда проверить снова? В формате ГГГГ-ММ-ДД',n.checkDate||'');if(d===null)return;await write(cs.map(x=>x.id===id?{...x,lastResult:next,checkDate:d||today(),updatedAt:new Date().toISOString()}:x));await render();toast('Контроль обновлён','success')}});
    ...
    const openControl=async preset=>{
      const m=document.createElement('div');m.className='modal open';m.innerHTML=`<div class="backdrop"></div><div class="sheet"><div class="sheet-head"><div><div class="eyebrow">Контроль</div><h2>${preset?'Поставить на контроль':'Новый контроль'}</h2></div><button class="circle-btn" data-close>×</button></div><form id="controlForm"><label>Что нужно держать на контроле<input id="cTitle" required placeholder="Например: дверь не готова — уточнить готовность" value="${esc(preset?.title||'')}"></label><label>Подробности<textarea id="cText" rows="4" placeholder="Что именно нужно выяснить?">${esc(preset?.text||'')}</textarea></label><div class="form-row"><label>Когда проверить<input id="cDate" type="date" value="${esc(preset?.checkDate||today())}" required></label><label>Телефон<input id="cPhone" type="tel" value="${esc(preset?.phone||'')}"></label></div><button class="primary wide" type="submit">Поставить на контроль</button></form></div>`;document.body.append(m);m.querySelector('.backdrop').onclick=()=>m.remove();m.querySelector('[data-close]').onclick=()=>m.remove();m.querySelector('#controlForm').onsubmit=async e=>{e.preventDefault();const cs=await read(),item={id:preset?.id||uid(),jobId:preset?.jobId||'',title:m.querySelector('#cTitle').value.trim(),text:m.querySelector('#cText').value.trim(),checkDate:m.querySelector('#cDate').value,phone:m.querySelector('#cPhone').value.trim(),client:preset?.client||'',address:preset?.address||'',store:preset?.store||'',archived:false,lastResult:preset?.lastResult||'',createdAt:preset?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};await write([...cs.filter(x=>x.id!==item.id),item]);m.remove();await render();toast('Поставили на контроль','success')};
    };
    ...
    const addControlButton=()=>{
      ...
      wrap.hidden=!active;
      if(active&&!wrap.dataset.bound){wrap.dataset.bound='1';wrap.innerHTML='<div>Если дверь не готова и новой даты пока нет — не оставляйте выезд просто отменённым.</div><button type="button" id="putControlBtn">＋ Поставить на контроль</button>';wrap.querySelector('#putControlBtn').onclick=async()=>{const j={jobId:document.querySelector('#jobId').value,title:`${document.querySelector('#jobClient').value||'Клиент'} — узнать готовность двери`,text:document.querySelector('#jobComment').value||'Уточнить готовность двери и после этого назначить новую дату.',checkDate:today(),phone:document.querySelector('#jobPhone').value||'',client:document.querySelector('#jobClient').value||'',address:document.querySelector('#jobAddress').value||'',store:document.querySelector('#jobStore').value||''};openControl(j)}};
    };
