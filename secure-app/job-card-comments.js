...
function renderComments() {
  const host = document.querySelector('#todayList');
  if (!host) return;

  host.querySelectorAll('.job-card[data-job-card]').forEach(card => {
    card.querySelectorAll('.job-comment').forEach(el => el.remove());
    card.querySelectorAll('.note-line').forEach(el => el.remove());
...
function watchTodayList() {
  if (listObserver) return;
  const host = document.querySelector('#todayList');
  if (!host) {
    requestAnimationFrame(watchTodayList);
    return;
  }
