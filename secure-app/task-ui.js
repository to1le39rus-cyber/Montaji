/* Montaji AA — Task UI v3. One visible task flow; no note modal bridge. */
(() => {
  ...
  async function saveTask(title, details) {
    // Reuse the already initialized Firebase app. No second data model and no note UI.
    ...
    await fs.runTransaction(db, async tx => {
      ...
      const item = { id: crypto.randomUUID?.() || `${Date.now()}-${Math.random()}`, title:`☐ ${title}`, text:details||title, archived:false, updatedAt:new Date().toISOString() };
      tx.set(ref,{data:{notes:[...notes,item]},version:1,updatedAt:fs.serverTimestamp(),updatedBy:auth.currentUser.uid},{merge:true});
    });
  }
