...
test('Clients groups jobs by normalized phone',()=>{
 const model=buildClientsModel({state:{jobs:[
  ...
  {id:'2',client:'Иван П.',phone:'79001112233',date:'2026-09-19',price:7000,status:'Запланировано'},
...
test('Cancelled jobs do not create client records',()=>{
 const model=buildClientsModel({state:{jobs:[
  {id:'1',client:'Удалённый',phone:'+79990000000',date:'2026-09-19',status:'Отменен'}
 ]}});
...
test('Client history keeps older jobs when newer job was processed first',()=>{
 const model=buildClientsModel({state:{jobs:[
  {id:'new',client:'Иван П.',phone:'79001112233',date:'2026-09-19',price:7000,status:'Запланировано'},
