...
test('job lifecycle commands preserve identity and history fields',()=>{
 ...
 assert.ok(job.id); assert.equal(job.status,'Запланировано');
 ...
 const cancelled=cancelJob(job,'Клиент отменил'); assert.equal(cancelled.status,'Отменен'); assert.ok(cancelled.cancelledAt); assert.equal(cancelled.cancelReason,'Клиент отменил');
 const moved=rescheduleJob(job,'2026-09-20'); assert.equal(moved.date,'2026-09-20'); assert.equal(moved.status,'Перенесен');
