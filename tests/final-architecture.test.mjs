...
test('income is based on completed work, not planned work',()=>{assert.match(app,/js\.filter\(done\)\.reduce/);assert.match(app,/state\.jobs\.filter\(x=>!cancelled\(x\)&&done\(x\)/);assert.match(app,/type==='Доп\. доход'\?'Выполнен'/)});
test('exact expense amounts are accepted',()=>{assert.match(app,/type=\\"number\\" min=\\"0\.01\\" step=\\"1\\"/);assert.match(app,/\!\(a>0\)/)});
