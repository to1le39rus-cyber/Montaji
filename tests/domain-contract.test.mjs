...
test('cancelled and incomplete jobs are not income',()=>{
  assert.equal(isIncomeEligible({status:'Отменен',price:100}),false);
  assert.equal(isIncomeEligible({status:'Запланировано',price:100}),false);
...
test('additional income is not montage workload',()=>{
  const jobs=[{date:'2026-09-19',type:'Монтаж',status:'Запланировано',slot:'1'},{date:'2026-09-19',type:'Доп. доход',status:'Запланировано',slot:'2'}];
  assert.equal(montageJobsForDate(jobs,'2026-09-19').length,1);
});
test('multiple montages per day are supported',()=>{
  const jobs=Array.from({length:5},(_,i)=>({date:'2026-09-19',type:'Монтаж',status:'Запланировано',slot:String((i%3)+1)}));
  assert.equal(montageJobsForDate(jobs,'2026-09-19').length,5);
});
test('next preset is convenience only',()=>{
  const jobs=Array.from({length:4},(_,i)=>({date:'2026-09-19',type:'Монтаж',status:'Запланировано',slot:String(i+1)}));
...
test('measurement conversion preserves relation and financial fields',()=>{
  ...
  const j=buildConvertedMontage(m,{date:'2026-09-20',status:'Запланировано',price:10000},'j1');
...
test('finance totals separate income, debt and expenses',()=>{
  const jobs=[{status:'Выполнен',type:'Монтаж',price:100,paid:false},{status:'Отменен',type:'Монтаж',price:900,paid:false}];
