...
test('Schedule supports more than three montage jobs on one date',()=>{
 const jobs=Array.from({length:5},(_,i)=>({
  ...
  status:i<2?'Выполнен':'Запланировано',slot:String((i%3)+1)
 }));
...
test('cancelled jobs are absent from Schedule',()=>{
 const model=buildScheduleModel({state:{jobs:[
  {id:'1',date:'2026-09-19',type:'Монтаж',status:'Отменен',slot:'1'},
  {id:'2',date:'2026-09-19',type:'Монтаж',status:'Запланировано',slot:'2'}
 ]},date:'2026-09-19'});
