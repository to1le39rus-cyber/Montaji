...
let ready=false,applying=false,lastJson='',db=null,ref=null,fsMod=null;
...
async function writeRemote(){
  if(!ready||!db||!ref||applying)return;
  const json=localStorage.getItem(DATA_KEY)||'';
  if(!json)return;
  try{
    const data=JSON.parse(json);
    const at=Math.max(Date.now(),stamp()+1);
    localStorage.setItem(SYNC_KEY,String(at));
    await fsMod.setDoc(ref,{data,updatedAt:at,updatedBy:clientId});
    lastJson=json;
    status('Firebase · сохранено');
  }catch(e){console.warn('sync write',e);status('Офлайн · данные сохранены на телефоне')}
}
window.syncNow=writeRemote;
async function start(){
  try{
    ...
    const app=appMod.initializeApp(cfg,'montaji-aa-sync');
    const auth=authMod.getAuth(app);
    db=fsMod.getFirestore(app);
    await authMod.signInAnonymously(auth);
    ready=true;
    ref=fsMod.doc(db,'appData','shared');
    status('Firebase · подключение…');
    const snap=await fsMod.getDoc(ref);
    const local=read(),localAt=stamp();
    if(snap.exists()){
      const remote=snap.data(),remoteAt=Number(remote.updatedAt||0);
      if(remoteAt>localAt&&remote.data?.jobs){
        applying=true;
        localStorage.setItem(DATA_KEY,JSON.stringify(remote.data));
        localStorage.setItem(SYNC_KEY,String(remoteAt));
        applying=false;
        lastJson=localStorage.getItem(DATA_KEY)||'';
        location.reload();
        return;
      }
      if(local?.jobs&&localAt>remoteAt)await fsMod.setDoc(ref,{data:local,updatedAt:localAt,updatedBy:clientId});
    }else if(local?.jobs){
      const at=Math.max(Date.now(),localAt+1);
      localStorage.setItem(SYNC_KEY,String(at));
      await fsMod.setDoc(ref,{data:local,updatedAt:at,updatedBy:clientId});
    }
    fsMod.onSnapshot(ref,s=>{
      if(!s.exists()||applying)return;
      const r=s.data(),remoteAt=Number(r.updatedAt||0),localAt2=stamp();
      if(remoteAt>localAt2&&r.data?.jobs){
        applying=true;
        localStorage.setItem(DATA_KEY,JSON.stringify(r.data));
        localStorage.setItem(SYNC_KEY,String(remoteAt));
        applying=false;
        lastJson=localStorage.getItem(DATA_KEY)||'';
        location.reload();
