const APP_URL = new URL('app.js?runtime=20260831-auth-savefix-4', location.href);
...
async function boot(){
  ...
  // Hard-stop native form navigation. Safari was reloading the page before
  // Firebase could finish the sign-in handler.
  document.addEventListener('submit',e=>{if(e.target?.id==='authForm')e.preventDefault()},true);
  ...
  source=source.replace(/function bindAuth\(\)\{[\s\S]*?\nfunction currentNotesData/,`function bindAuth(){
    const form=$('#authForm');
    if(form&&!form.dataset.bound){form.dataset.bound='1';form.addEventListener('submit',async e=>{e.preventDefault();e.stopPropagation();const email=$('#authEmail').value.trim(),pass=$('#authPassword').value;if(!online)return authMessage('Нет интернета.',true);if(!email||!pass)return authMessage('Введите email и пароль.',true);authMessage('Входим…');const b=form.querySelector('button[type="submit"]');if(b){b.disabled=true;b.textContent='Входим…'}try{await F.authMod.signInWithEmailAndPassword(auth,email,pass)}catch(err){authMessage(authError(err),true)}finally{if(b){b.disabled=false;b.textContent='Войти'}}},{passive:false});}
    $('#signUpBtn')?.addEventListener('click',async()=>{const email=$('#authEmail').value.trim(),pass=$('#authPassword').value;if(!email||!pass)return authMessage('Введите email и пароль.',true);if(pass.length<6)return authMessage('Пароль должен быть не короче 6 символов.');try{await F.authMod.createUserWithEmailAndPassword(auth,email,pass);authMessage('Аккаунт создан. Входим…')}catch(err){authMessage(authError(err),true)}});
...
function startRealtime`);
  ...
  source=source.replace(/async function saveShared\(mutator\)\{[\s\S]*?\}\nasync function saveNotes/,`async function saveShared(mutator){
    ...
    const ref=F.doc(db,...SHARED_DOC);let snap;try{snap=await F.getDocFromServer(ref)}catch(e){snap=await F.getDoc(ref)}
    const current=snap.exists()?normalize(snap.data().data):emptyState(),next=normalize(await mutator(current));
    try{await F.setDoc(ref,{data:next,version:5,updatedAt:F.serverTimestamp(),updatedBy:user.uid},{merge:true})}catch(primaryErr){try{await F.runTransaction(db,async tx=>{const s=await tx.get(ref),latest=s.exists()?normalize(s.data().data):emptyState(),latestNext=normalize(await mutator(latest));tx.set(ref,{data:latestNext,version:5,updatedAt:F.serverTimestamp(),updatedBy:user.uid},{merge:true})})}catch(txErr){throw new Error('Не удалось сохранить ['+(txErr?.code||primaryErr?.code||'unknown')+']')}}
    state=next;render();
  }
async function saveNotes`);

  source=source.replace(/async function saveNotes\(mutator\)\{[\s\S]*?\}\nfunction jobsForDate/,`async function saveNotes(mutator){
    if(!online||!user)throw new Error('Нет соединения с общей базой');const ref=F.doc(db,...NOTES_DOC),sharedRef=F.doc(db,...SHARED_DOC),snap=await F.getDoc(ref).catch(()=>null),cur={notes:currentNotesData(snap)},next=await mutator(JSON.parse(JSON.stringify(cur))),safe=Array.isArray(next?.notes)?next.notes:[];
    try{await F.setDoc(ref,{data:{notes:safe},version:1,updatedAt:F.serverTimestamp(),updatedBy:user.uid},{merge:true})}catch(e){await F.runTransaction(db,async tx=>{const s=await tx.get(sharedRef),data=s.exists()?s.data()?.data||{}:{};tx.set(sharedRef,{data:{...data,notes:safe},version:5,updatedAt:F.serverTimestamp(),updatedBy:user.uid},{merge:true})})}notes=safe;renderNotes();
  }
function jobsForDate`);
  ...
  const themeCss=document.querySelector('link[href*="premium-field-tech.css"]');if(themeCss)themeCss.href='premium-field-tech.css?v=20260821-6';
  ...
  const blob=new Blob([source],{type:'text/javascript'}),url=URL.createObjectURL(blob);try{await import(url)}finally{URL.revokeObjectURL(url)}
}
