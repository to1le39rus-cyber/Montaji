...
async function boot(){
  ...
  /* Sandbox: the old notes stack is deliberately disabled. A new notes module will
     be introduced separately, with its own data model and storage path. */
  ...
  source=source.replace(/let F, auth, db, user, unsubscribeShared=null, unsubscribeNotes=null; let state=emptyState\(\), notes=\[\], serverReady=false, online=navigator\.onLine;/,
    "let F, auth, db, user, unsubscribeShared=null; let state=emptyState(), serverReady=false, online=navigator.onLine;");
  source=source.replace(/function currentNotesData\([\s\S]*?\nasync function loadServer/,`async function loadServer(){
    if(!user||!online){serverReady=false;state=emptyState();render();status('Нет интернета · данные не загружены','offline');return false;}
    status('Подключаем общую базу…');
    try{
      const snap=await F.getDocFromServer(F.doc(db,...SHARED_DOC));
      state=snap.exists()?normalize(snap.data().data):emptyState();
      serverReady=true;render();status('● Общая база · синхронизировано','online');return true;
    }catch(err){
      console.error(err);serverReady=false;state=emptyState();render();status('База недоступна','offline');toast('Не удалось получить данные с сервера.','error');return false;
    }
  }
async function startRealtime`);
  source=source.replace(/function startRealtime\(\)\{[\s\S]*?\nasync function saveShared/,`function startRealtime(){
    ...
    unsubscribeShared=F.onSnapshot(F.doc(db,...SHARED_DOC),snap=>{
      if(!online)return;state=snap.exists()?normalize(snap.data().data):emptyState();serverReady=true;render();status('● Общая база · обновлено','online');
    },()=>{serverReady=false;status('Нет связи с общей базой','offline');toast('Потеряна связь с общей базой','error')});
  }
async function saveShared`);
  ...
  cleanup.textContent='.sandbox-notes-old,.mega-notes,#controlSection{display:none!important}';
